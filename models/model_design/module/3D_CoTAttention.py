...
if __name__ == '__main__':
    block = CoTAttention3D(32)  # 创建一个输入通道数为64的CoTAttention3D实例
    input = torch.rand(1, 32, 64, 64, 64)  # 创建一个随机输入 (batch_size, channels, depth, height, width)
    output = block(input)  # 通过CoTAttention3D模块处理输入

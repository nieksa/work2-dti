...
class NonLocal(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        pool_theta: nn.Module,
        pool_phi: nn.Module,
        pool_g: nn.Module,
        pool_input:nn.Module,
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        N, C, T, H, W = x.size()
        x_identity = x
        x_identity = self.pool_input(x_identity)
        theta = self.conv_theta(x)
        theta = self.pool_theta(theta)
...
def create_nonlocal(
    *,
    ...
    return NonLocal(
        ...
        pool_theta= nn.AvgPool3d((1, 1, 1)),
        pool_phi= nn.AvgPool3d((1, 1, 1)),
        pool_g= nn.AvgPool3d((1, 1, 1)),
        pool_input= nn.AvgPool3d((1, 1, 1)),

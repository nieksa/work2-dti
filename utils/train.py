...
def train_contrastive_epoch(model, train_loader, optimizer, scheduler, device, epoch):
    ...
    for batch_idx, (data, labels) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        step += 1
        # 4. 计算对比损失
        ssim_loss = compute_contrastive_heatmap_ssim_loss(fa_map, mri_map, pos_pairs, neg_pairs, margin=1.0)
        ...
        # )
        total_loss = contrastive_loss_val + classification_loss + fa_loss + mri_loss + ssim_loss
        # 7. 反向传播 + 优化
        total_loss.backward()
        optimizer.step()
